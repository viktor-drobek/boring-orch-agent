...
def handler_type(store: Store, auth_token: str):
    ...
    class Handler(BaseHTTPRequestHandler):
        ...
        def _handle(self, route, mutating: bool):
            """Run one route and always answer with the documented JSON error shape."""
            if not self._authorized() or not self._same_origin(mutating):
                return
            try:
                ...
                # A body value of the wrong type or format reached a lower layer.
                # Internal details stay out of the reply.
                self._reply(400, {"error": "invalid_request",
                                  "message": "Request contains a value of an unsupported type or format"})
            except Exception:
                try:
                    self._reply(500, {"error": "internal_error", "message": "Internal server error"})
                except OSError:
                    pass

        def _same_origin(self, mutating: bool) -> bool:
            """Without a bearer token, blunt DNS rebinding and simple cross-site posts."""
            if auth_token:
                return True
            if not _loopback_host_header(self.headers.get("Host", "")):
                self._reply(403, {"error": "forbidden",
                                  "message": "A token-less API accepts only a loopback Host header"})
                return False
            if mutating and not _json_content_type(self.headers.get("Content-Type", "")):
                self._reply(415, {"error": "unsupported_media_type",
                                  "message": "Mutating requests require Content-Type: application/json"})
                return False
            return True
        ...
        def _authorized(self) -> bool:
            ...
            expected = ("Bearer " + auth_token).encode("utf-8", "surrogateescape")
        ...
        def do_GET(self):
            self._handle(self._get, mutating=False)

        def do_POST(self):
            self._handle(self._post, mutating=True)
        ...
        def _post(self, parts):
            ...
            if parts == ["api", "v1", "discovery", "approve"]:
                # Approval is operator consent to run an executable or to send a
                # credential to a base URL. A request body cannot carry that consent.
                return self._reply(403, {"error": "operator_only", "message":
                                         "Discovery approval is operator-only; use Discovery.approve() locally"})
