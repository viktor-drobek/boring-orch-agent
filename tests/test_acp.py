...
class ACPIsolationTests(unittest.TestCase):
    ...
    def test_security_rejects_bypass_and_disables_extensions(self):
        with self.assertRaisesRegex(ACPError, "bypass"):
            secure_agent({"permission_mode": "bypass"}, str(self.state))
...
class ACPOperationTests(unittest.TestCase):
    ...
    def test_prepare_launch_records_all_contracts(self):
        task = {"workspace": self.root_workspace(), "state_path": self.state_path(), "sandbox": "workspace-write",
                "mode": "interactive", "model": "fixture-1", "budget": {"deadline_seconds": 3},
                "budget_capabilities": {"wall_clock": True, "process_group_kill": True,
                                        "per_call": False, "step_count": False, "token_reporting": False}}
        ...
        self.assertIn("HOME", plan.environment)

    def root_workspace(self):
